...
public interface RestauranteCrudRepository extends CrudRepository<Restaurante,Integer> {
    ...
    @Query(value="SELECT * FROM restaurante WHERE tipo_de_restaurante = ?",nativeQuery = true)
    List<Restaurante> getTipoRestaurante(int tipo_de_restaurante);//como se busca usando sql
